...
class TestMap:
    ...
    def test_move(self):
        ...
        m = ma.Map(map)
        rd.seed(5)
        m.populate_map((1, 1), [ma.Carnivore(
            age=10, weight=50) for _ in range(100)])
        m.populate_map((1, 1), [ma.Herbivore(
            age=15, weight=30) for _ in range(10)])

        print(m.island[1, 1].total_pop())
        print(m.island[1, 2].total_pop())
        m.move()
        print(m.island[1, 1].total_pop())
        print(m.island[1, 2].total_pop())

        assert m.island[1, 1].total_pop() == 62
        assert m.island[1, 2].total_pop() == 48
        assert m.island[1, 2].pop_total[:].animal_moved is False
